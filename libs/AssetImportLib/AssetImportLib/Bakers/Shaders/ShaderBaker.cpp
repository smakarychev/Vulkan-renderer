...
slang::ISession& getSession(const ShaderImportSettings& settings, const assetlib::ShaderLoadInfo::Variant& variant)
{
...
public:
    UniformTypeReflector(const Context& ctx, const ShaderImportSettings& settings) : m_Ctx(&ctx), m_Settings(&settings) {}
...
private:
    ...
    const ShaderImportSettings* m_Settings{nullptr};
...
{
public:
    ShaderReflector(const Context& ctx, const ShaderImportSettings& settings, bool usesTextureHeap)
...
ShaderBaker::ShaderBaker(const std::shared_ptr<Context>& ctx, const ShaderImportSettings& settings)
    : m_Ctx(ctx), m_Settings(&settings)
{
}
