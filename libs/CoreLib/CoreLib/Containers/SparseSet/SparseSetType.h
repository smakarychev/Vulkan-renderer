...
template <typename St, typename Dt>
constexpr St SparseSetType<St, Dt>::Erase(Dt value)
{
    auto&& [gen, index] = Traits::Decompose(value);
    auto* sparseSet = TryGet(index);
    auto mappedIndex = Math::fastMod(index, SPARSE_SET_PAGE_SIZE);
    const St denseToRemove = (*sparseSet)[mappedIndex];
    ASSERT(sparseSet, "Invalid value")
    ASSERT(denseToRemove != NON_ELEMENT, "No such value")
    ...
    if (m_Dense.size() > 1)
    {
        ...
        std::swap(m_Dense[denseToRemove], m_Dense.back());
