...
public:
    ...
    constexpr Iterator begin() { return Iterator(m_Dense, (u32)(m_Dense.size() - 1)); }
...
constexpr bool SparseSetType<St, Dt>::Has(Dt value) const
{
    ...
    auto mappedIndex = Math::fastMod(index, SPARSE_SET_PAGE_SIZE);
    const St sparseI = (*sparseSet)[mappedIndex];

    return sparseI < (St)m_Dense.size() && m_Dense[sparseI] == value;
}
