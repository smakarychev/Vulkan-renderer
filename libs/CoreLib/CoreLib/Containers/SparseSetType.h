...
public:
    ...
    template <typename PushCallback>
    constexpr St insert(Dt value, PushCallback callback = [](St){}) { return Insert(value, std::move(callback)); }
    
    constexpr void Erase(Dt value);
    template <typename PopCallback, typename SwapCallback>
    constexpr void Erase(Dt value, PopCallback popCallback = [](St){}, SwapCallback swapCallback = [](St, St) {});
    constexpr void erase(Dt value) { Erase(value); }
    template <typename PopCallback, typename SwapCallback>
    constexpr void erase(Dt value, PopCallback popCallback = [](St){}, SwapCallback swapCallback = [](St, St) {})
    {
        Erase(value, std::move(popCallback), std::move(swapCallback));
    }
    
    /* checks if the value, that was generated by this set, still belongs to it */ 
    constexpr bool Has(Dt value) const;
    constexpr bool has(Dt value) const { return Has(); }
...
template <typename St, typename Dt>
void SparseSetType<St, Dt>::Clear()
{
    m_Dense.clear();
    m_SparsePaged.clear();
}
...
template <typename St, typename Dt>
constexpr void SparseSetType<St, Dt>::Erase(Dt value)
{
    auto&& [gen, index] = Traits::Decompose(value);
...
template <typename St, typename Dt>
template <typename PopCallback, typename SwapCallback>
constexpr void SparseSetType<St, Dt>::Erase(Dt value, PopCallback popCallback, SwapCallback swapCallback)
{
    auto&& [gen, index] = Traits::Decompose(value);
    auto* sparseSet = TryGet(index);
    auto mappedIndex = Math::fastMod(index, SPARSE_SET_PAGE_SIZE);
...
template <typename St, typename Dt>
constexpr bool SparseSetType<St, Dt>::Has(Dt value) const
{
    auto&& [gen, index] = Traits::Decompose(value);
    auto* sparseSet = TryGet(index);
    auto mappedIndex = Math::fastMod(index, SPARSE_SET_PAGE_SIZE);
...
template <typename St, typename Dt>
constexpr St SparseSetType<St, Dt>::IndexOf(Dt value) const
{
    auto&& [gen, index] = Traits::Decompose(value);
    auto* sparseSet = TryGet(index);
...
template <typename St, typename Dt>
constexpr Dt& SparseSetType<St, Dt>::operator[](Dt value)
{
    return const_cast<Dt&>(const_cast<const SparseSetType&>(*this).operator[](value));
}
...
template <typename St, typename Dt>
constexpr const std::vector<St>* SparseSetType<St, Dt>::TryGet(u32 index) const
{
    u32 pageNum = index >> SPARSE_SET_PAGE_SIZE_LOG;
...
template <typename St, typename Dt>
constexpr std::vector<St>* SparseSetType<St, Dt>::TryGet(u32 index)
{
